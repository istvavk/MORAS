class Parser:
    def __init__(self):
        ...
        self._labels = []     # Prvo napravimo listu oznaka
    ...
    def _parseLine(self, line, n):
        ...
        if l[0] == "push":
            if len(l) == 3 and l[2].isnumeric():
                return "//" + " ".join(l) + "\n" + self._push(l[1], l[2], n)
            else:
                self._flag = False
                Parser._error("Parser", n, "Invalid argument for push command. Should be non-negative integer.");
                return ""
            ...
            if len(l) == 3 and l[2].isnumeric():
                ...
                self._flag = False
                Parser._error("Parser", n, "Invalid argument for pop command. Should be non-negative integer.");
                return ""
    ...
    # Svaku oznaku koja se definira u programu dodajemo u listu oznaka
    def _label(self, lab, n):
        self._labels.append(lab)
        return "(" + self._func + "$" + lab + ")"
    
    # U metodama '_goto' i '_ifgoto' provjeravamo jel se u listi oznaka nalazi
    # i ona koja se koristi u naredbi, ako se ne nalazi, ispisat cemo poruku o pogresci
    # i postavit flag na False
    def _goto(self, lab, n):
        if lab not in self._labels:
            self._flag = False
            Parser._error("Parser", n, "Undefined label for goto command")
            return "@" + self._func + "$" + lab + "\n0;JMP"
    
    def _ifgoto(self, lab, n):
        if lab not in self._labels:
            self._flag = False
            Parser._error("Parser", n, "Undefined label for if-goto command")
            return "@SP\nAM=M-1\nD=M+1\n@" + self._func + "$" + lab + "\nD;JEQ"
